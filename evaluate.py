...
line_dict = {
    'NSGA-II': ['blue', '--'],
    'NSGA-II w/ PSI k=1': ['green', 'solid'], 'NSGA-II w/ PSI k=2': ['orange', 'solid'],
    'NSGA-II w/ TF-IPS k=1': ['red', 'solid']
}
...
def get_nEvals_and_IGD(pop_size):
    ...
    for exp in experiments_list:
        ...
        for i in range(N_RUNS):
            ...
            tmp_data = np.array(p.load(open(os.path.join(path_result, '#Evals_runningtime_IGD_each_gen.p'), 'rb')))
...
def visualize_nEvals_and_IGD(logX=None, logY=None):
    ...
    for i, exp in enumerate(experiments_list):
        ...
        variant_configuration = hyperparameters[1:]
        label = algorithm_name
        if variant_configuration[0] == 'True':
            label = label + ' w/ '
            if variant_configuration[2] == 'True':
                label = label + f'TF-PSI k={variant_configuration[1]}'
            else:
                label = label + f'PSI k={variant_configuration[1]}'
    ...
    plt.legend(bbox_to_anchor=(-0.15, -0.06, 1.3, .02), mode='expand', fontsize=8, ncol=3, frameon=False)

    title = f'{problem_name} | x-axis: #Evals'
    figure_name = f'{PATH_RESULTS}/IGD.jpg'
...
def visualize_nEvals_and_Hypervolume(logX=None, logY=None):
    ...
    title = f'{problem_name} | x-axis: #Evals'
    figure_name = f'{PATH_RESULTS}/Hypervolume.jpg'
...
if __name__ == '__main__':
    ...
    N_RUNS = 31
    checked_lst = ['IGD', 'Hypervolume', 'nEvals', 'jpg']
    """ =========================================== """
    exp_0 = None
    problem_name = None
    dataset = None
