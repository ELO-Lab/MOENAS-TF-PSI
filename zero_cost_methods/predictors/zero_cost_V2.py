...
class ZeroCostV2(Predictor):
    def __init__(self, config, batch_size=64, method_type='synflow'):
        ...
        self.arch2score_dict = {}

    def pre_process(self):
        self.train_loader, _, _, _, _ = get_train_val_loaders(self.config, mode='train')

    def query_(self, arch_mod, arch_ori, nTimes=1):
        try:
            hashKey = get_hashKey(arch=arch_ori, problem_name=self.config['search_space'])
            return self.arch2score_dict[hashKey]
        except KeyError:
            if self.config['search_space'] == 'NASBench101':
                ...
                # Required format: |{}|+|{}|{}|+|{}|{}|{}|
                network = nas201_arch.get_model_from_arch_str(arch_mod, self.num_classes)
                arch_2 = nas201_arch.get_arch_str_from_model(network)
                if arch_mod != arch_2:
                    print(f'Value Error: orig_arch={arch_mod}, convert_arch={arch_2}')
                    measure_score = -10e8
                    return measure_score
            elif self.config['search_space'] == 'NASBench301':
                # Required format: compact_arch (genotype)
                genotype = convert_compact_to_genotype(arch_mod)
                arch_config = {
                    "name": "darts",
                    "C": 32,
                    "layers": 8,
            ...
            for _ in range(nTimes):
                ...
                for key in score:
                    if math.isnan(score[key]):
                        score[key] = -1e8
                    score_lst.append(score[key])
            # if math.isnan(score):
            #     score = -1e8
            torch.cuda.empty_cache()
            hashKey = get_hashKey(arch=arch_ori, problem_name=self.config['search_space'])
            self.arch2score_dict[hashKey] = sum(score_lst)/len(score_lst)
            return self.arch2score_dict[hashKey]